...
const getStatusCounts = (testRun) => {
    ...
    testRun?.map((module) => {
        ...
        if (testNodes) {
            ...
            ignored += testNodes?.filter(
                (testNode) => testNode.status === 'IGNORED'
            ).length
            warning += testNodes?.filter(
                (testNode) => testNode.status === 'WARNING'
            ).length
        } else {
                    ...
                    testNodes.push({
                        ...
                        testCaseDescription:
                            modules[0].testNodes[mt].testNode[0]
                                .testCaseDescription,
                        testCaseID:
                            modules[0].testNodes[mt].testNode[0].testCaseID,
...
const getModulesDataWithSteps = async (
    ...
    try {
        ...
        if (releaseId !== null && includeSteps) {
            project = await TestCaseSteps.findOne({
                projectId,
                releaseIds: { $in: [releaseId] },
            }).sort({ createdAt: -1 })
        } else if (jobId !== null && includeSteps) {
            project = await TestCaseSteps.findOne({
                projectId,
                jobIds: { $in: [jobId] },
            })
        }
        if (project === null && includeSteps) {
        ...
        const modules = uniqueModules.map((module) => {
            ...
            const testNodes = module.testNodes.map((testNode) => {
                ...
                if (includeSteps && testCaseSteps)
                    newTestCaseSteps = [...newTestCaseSteps, ...testCaseSteps]

                if (excludeKeys) {
                    newTestCaseSteps = newTestCaseSteps.filter((step) => {
                        // Get all keys except _id
                        const stepKeys = Object.keys(step).filter(
                            (k) => k !== '_id'
                        )

                        // Keep the step if none of its keys are in excludeStepKeys
                        return !stepKeys.some((k) => ExcludeKeys.includes(k))
                    })
                }

                testNode = JSON.parse(JSON.stringify(testNode))

                let newTestNode = null
                if (includeSteps)
                    newTestNode = {
                        ...testNode?.testNode[0],
                ...
                if (includeSteps) testStepCount += newTestCaseSteps.length
...
const findValue = (obj, key, excludeKeys = [], seen = new WeakSet()) => {
    ...
    if (Array.isArray(obj)) {
        for (const item of obj) {
            const result = findValue(item, key, excludeKeys, seen)
    ...
    for (const k of Object.keys(obj)) {
        ...
        if (typeof obj[k] === 'object') {
            const result = findValue(obj[k], key, excludeKeys, seen)
            if (result !== undefined) return result
        }
    }
