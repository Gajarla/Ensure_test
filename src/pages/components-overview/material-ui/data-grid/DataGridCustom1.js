...
import { JOB_STATUS, IndexedDB, INDEXEDDB_KEYS, STATUS_CODES as STATUS } from '../../../../Constants';
...
DataGridCustom1.propTypes = {
  testCases: PropTypes.array
};

export default function DataGridCustom1({ testCases }) {
  ...
  const CustomExportButton = () => {
    ...
    const handleExport = async () => {
      ...
      if (obj.jobStatus === 'Completed') {
        const summary = obj;
        const result = { details, summary, defectList };
        exportTestCasesWithSteps(result, selectedOptions);
      } else {
        setDialogOpen(true);
      }
      // setDialogOpen(false);
    ...
    return (
      <>
        <Button
          ...
          // The below will be used when the csv file
          // requires logs and screenshot links in it
          // onClick={() => setDialogOpen(true)}
          variant="text" // Matches the appearance of default GridToolbarExport
