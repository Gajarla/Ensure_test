    ...
    try {
        ...
        responseTransformer.findByAndUpdateCb(
            ...
            async (newProject) => {
                ...
                // Project Creation in ADO

                const adoRequestBody = {
                ...
                if (project && project._id) {
                    if (ADO_ENABLED) {
                        const response = await axios.post(
                            `${process.env.ADO_INTEGRATION_HOST}/api/projects/createAdoProject`,
                            adoRequestBody,
                            { headers }
                        )
                    }
        ...
        try {
            ...
            if (req.params.order === 'asc' || req.params.order === 'desc') {
                ...
                const projects = await Project.find(filterText)
                    ...
                    .limit(limitno)
                    .select(
                        '_id name team status requestFiles createdAt updatedAt templateID company modulesCount'
                    )
...
//                             const formattedDuration = moment
//                                 .utc(
//                                     moment(executionEnd).diff(
//                                         moment(executionStart)
//                                     )
//                                 )
//                                 .format('m[m] s[s]')
//                             const moduleId = modules[0]?.moduleID
...
// Search Test Runs with Pagination
router.get(
    '/v1/AllJobs/:projectID/:SortBy/:order/:pagesize/:pageno',
    async (req, res) => {
        const warnings = []
        try {
            const { projectID, SortBy, order, pagesize, pageno } = req.params
            const searchTerm =
                req.query?.searchTerm?.trim()?.toLowerCase() || '' // ✅ searchTerm
            const sortOrder = order === 'asc' ? 1 : -1
            ...
            if (jobs?.length === 0)
                return responseTransformer.dbResponseTransformer(
                    null,
                    [],
                    'list jobs',
                    res
            ...
            for (const job of jobs) {
                ...
                if (release) {
                    ...
                    // ✅ Construct testRun string for searching
                    const testRunString = `${moduleName}_${job.jenkinsJobID}`
                ...
                // ✅ End of loop: send response when all jobs processed
                if (jobs.length === newJobs.length + releasesSkipped) {
                    try {
                        // ✅ Apply search filter here (only on testRun)
                        ...
                        const totalCount = filteredJobs.length || 0
                        const paginatedTestRuns = filteredJobs.slice(
                            skip,
                            skip + limit
                        )
    ...
    try {
        ...
        await axios
            .post(
                ...
                'http://192.168.0.42:8081/process',
                formData,
                {
                    headers: {
...
router.get('/getTestCaseSteps/:companyId/:projectId', async (req, res) => {
    try {
        ...
        res.json(
            testCaseSteps
                ? testCaseSteps[testCaseSteps.length - 1]?.methods
                : null
        )
    } catch (error) {
...
//Modules with specific fields
router.get(
    '/v2/ModuleByPID/:projectID/:fetchTestCases?/:fetchTestSteps?',
    async (req, res) => {
        try {
            const { fetchTestCases = 'true', fetchTestSteps = 'true' } =
                req.params
            ...
            if (fetchTestCases !== 'true') {
                modulesWithCounts = modulesWithCounts?.map(
                    (modulesWithCount) => {
                        delete modulesWithCount.testNodes
                        delete modulesWithCount.testPlaceholders
                        return modulesWithCount
                    }
                )
            }

            if (fetchTestSteps !== 'true') {
                // Flatten all testCaseSteps from module.testNodes
                modulesWithCounts = modulesWithCounts?.map(
                    (modulesWithCount) => {
                        modulesWithCount.testNodes =
                            modulesWithCount.testNodes?.map((node) => {
                                return {
                                    ...node,
                                    testNode: node.testNode.map((tn) => {
                                        // remove testCaseSteps key
                                        const { testCaseSteps, ...rest } = tn
                                        return rest
                                    }),
                                }
                            })
...
router.get(
    '/v2/ModuleByPID/:projectID/:SortBy/:order/:pagesize/:pageno/:fetchTestCases?/:fetchTestSteps?',
    async (req, res) => {
        try {
            const {
                projectID,
            ...
            //     skip + limit
            // )
            const paginatedModules = modulesWithCounts
            ...
            // Call for getAllAdoTestPlans
            if (ADO_ENABLED) {
                ;(async () => {
                    try {
                        let project = await common.getProjectNameById(projectID)
                        let projectName = project.name
                        let projectCompany = project.company
                        let adoRequestBody = {
                            projectName,
                        }
                        const headers = {
                        ...
                        console.log(
                            `${process.env.ADO_INTEGRATION_HOST}/api/test-plans/getAllAdoTestPlans`
                        )
                        ...
                        const testplans = response.data.value.map((item) => ({
                            name: item.name,
                            areaPath: item.areaPath,
                        }))

                        const releaseNames = await Release.find({})
                            .select('releaseName')
                            .lean()

                        const savedReleases = releaseNames.map(
                            (r) => r.releaseName
                        )

                        const filteredTestPlans = testplans.filter(
                            (tp) => !savedReleases.includes(tp.name)
                        )

                        console.log(
                            'ADO plans missing in DB:',
                            filteredTestPlans
                        )
                        adoRequestBody = {
                            projectName,
                            testPlanName: filteredTestPlans[0].name,
                        }
                        response = await axios.post(
                            `${process.env.ADO_INTEGRATION_HOST}/api/test-suites/getAllAdoTestSuites`,
                            adoRequestBody,
                            { headers }
                        )
                        adoRequestBody = {
                            projectName,
                            testPlanName: response.data.value[0].name,
                            testSuiteName: response.data.value[1].name,
                        }
                        let testPlanName = response.data.value[0].name
                        let testSuiteName = response.data.value[1].name
                        response = await axios.post(
                            `${process.env.ADO_INTEGRATION_HOST}/api/test-suites/getAllTestCasesInAdoTestSuite`,
                            adoRequestBody,
                            { headers }
                        )
                        let testCaseNumbers = extractTestCaseIds(
                            response.data.value
                        )
                        adoRequestBody = {
                            projectName,
                        ...
                        response = await axios.post(
                            `${process.env.ADO_INTEGRATION_HOST}/api/test-cases/getAdoTestCasesDetails`,
                            adoRequestBody,
                            { headers }
                        )
                        let testCasesObject = extractMultipleTestCaseDetails(
                            response.data
                        )
                        // Need to create Module in TE
                        let moduleCreationTeRequestBody = {
                            automationStatus: false,
                            businessProcess: '',
                            company: projectCompany,
                            // User details of raj
                            createdBy: '664040024dbc43cb2b8ee6c0',
                            email: 'raj@sailotech.com',
                            projectID,
                            suiteDescription: testSuiteName,
                            suiteName: testSuiteName,
                            testNodes: formatToTestNodes(testCasesObject),
                        }
                        // Create Module
                        response = await common.createModule(
                            moduleCreationTeRequestBody
                        )
                        ...
                        // Create Release
                        const module = await Module.findById(response.data._id)
                        const testNodeIds = []
                        const testCaseIDs = []
                        // Need to format the structure as below for a module for creating a release
                        module?.testNodes?.forEach((testCase) => {
                            testNodeIds.push(testCase?._id)
                            testCaseIDs.push(
                                `${testCase.testNode[0].testCaseID} - ${testCase.testNode[0].testCaseTitle}`
                            )
                        })
                        ...
                        let releaseCreationTeRequestBody = {
                            releaseName: testPlanName,
                            ...
                            scheduledOn: '',
                            modules: [moduleObj],
                            templateId: '657c1e5bf924a041d0c9c341',
                            createdBy: '664040024dbc43cb2b8ee6c0',
                            multiModuleSelection: 'Modules Only',
                        }
                        response = await common.createRelease(
                            releaseCreationTeRequestBody
                        )
                        if (response.status == true) {
                        ...
                        logger.info('Background ADO sync completed ✅')
                    } catch (adoError) {
                        logger.error(
                            `Error during ADO background call: ${adoError}`
                        )
                    }
...
function formatToTestNodes(inputTestCaseArray) {
    if (!Array.isArray(inputTestCaseArray)) {
        console.error('Input must be an array.')
        return []
    }

    return inputTestCaseArray.map((item, index) => {
        // 1. Format the steps into the desired 'manual' structure
        const formattedSteps = formatSteps(item.steps)

        // 2. Construct the final testNode object
        return {
            testNode: {
                // NOTE: testCaseID and dependsOn are not available in your input,
                // so we use placeholder/index values. You must update these
                // if the actual data is available elsewhere.
                testCaseID: item.id || 1 + index, // Using a placeholder/index ID
                testCaseTitle: item.title,
                testCaseDescription:
                    item.description || `Test Case for ${item.title}`, // Assuming description is not provided
                dependsOn: 'manual', // Placeholder as per your requirement
                testCaseSteps: formattedSteps,
            },
        }
    })
}
...
/**
 * Helper function to reformat the steps array.
 */
function formatSteps(stepsArray) {
...
function extractMultipleTestCaseDetails(adoWorkItemsArray) {
    if (!Array.isArray(adoWorkItemsArray)) {
        console.error('Invalid input: Expected an array of ADO Work Items.')
        return []
    }

    // Use map() to iterate over the array and apply the single-object extraction function
    const structuredDataArray = adoWorkItemsArray
        .map((adoWorkItem) => {
            // Call the existing function for each object in the array
            return extractTestCaseDetails(adoWorkItem)
        })
        .filter((item) => item !== null) // Remove any items that failed parsing (returned null)

    return structuredDataArray
}

function extractTestCaseIds(apiResponseValue) {
    if (!apiResponseValue || !Array.isArray(apiResponseValue)) {
        console.error('Invalid input: Expected an array.')
        return []
    }

    // First, map the array of objects to an array of URL strings.
    const urlArray = apiResponseValue
        .map((item) => {
            // This is the string you need: e.g., 'https://.../workItems/213'
            return item.testCase ? item.testCase.url : null
        })
        .filter((url) => url !== null) // Filter out any null values

    // Second, map the array of URL strings to an array of just the ID numbers.
    const testCaseNumbersArray = urlArray
        .map((url) => {
            // 💡 FIX: The variable 'url' is guaranteed to be a string here.
            // Calling .lastIndexOf('/') on a string is correct.
            const lastSlashIndex = url.lastIndexOf('/')

            if (lastSlashIndex !== -1) {
                // Extract the substring (the ID number)
                const numberString = url.substring(lastSlashIndex + 1)
    ...
    return testCaseNumbersArray
}

function extractTestCaseDetails(adoWorkItem) {
    if (!adoWorkItem || !adoWorkItem.fields) {
        console.error('Invalid ADO Work Item object.')
        return null
    }
    ...
    // If there are no steps, return the title and an empty array for steps
    if (!stepsXml) {
        return {
            title: title,
            steps: [],
    ...
    // Regular Expression to find and capture step details:
    // This looks for:
    // 1. The step ID (id="(\d+)")
    // 2. The Step Description and Expected Result (inside <parameterizedString>)
    const stepRegex =
        /<step id="(\d+)".*?<parameterizedString isformatted="true">(.*?)<\/parameterizedString><parameterizedString isformatted="true">(.*?)<\/parameterizedString>/g

    const steps = []
    let match

    // Use a loop to find all step matches in the XML string
    while ((match = stepRegex.exec(stepsXml)) !== null) {
        const stepId = match[1]
        // The descriptions can contain HTML entities like &lt; and &gt;
        // A simple function (defined below) is used to clean these up.
        const testStepDescription = cleanupHtmlEntities(match[2])
        const expectedResult = cleanupHtmlEntities(match[3])
...
// Helper function to clean up common HTML entities like &lt; and &gt;
function cleanupHtmlEntities(htmlString) {
...
router.get(
    '/v2/ModuleByPID/:projectID/:moduleID/:SortBy/:order/:pagesize/:pageno/:fetchTestCases?/:fetchTestSteps?',
    async (req, res) => {
        try {
            const {
                projectID,
            ...
            if (fetchTestCases !== 'true') {
                delete module.testNodes
                delete module.testPlaceholders
            }

            if (fetchTestSteps !== 'true') {
                // Flatten all testCaseSteps from module.testNodes
                module.testNodes = module.testNodes.map((node) => {
                    return {
                        ...node,
                        testNode: node.testNode.map((tn) => {
                            // remove testCaseSteps key
